import { HeartIcon, HomeIcon, PersonIcon } from "@radix-ui/react-icons";

export function Stepper({ step }: { step: "one" | "two" | "three" }) {
  return (
    <div className="flex flex-row justify-between px-9 items-center relative">
      ...
      >
        <HomeIcon
          className={`h-full w-full ${step !== "two" ? "text-black" : "text-white"}`}
        />
      </div>
      <div
        className={`w-10 h-10 rounded-full p-1 z-10 ${
          step !== "three" ? "bg-white border-pink-500 border-2" : ""
        }`}
      >
        <HeartIcon
          className={`h-full w-full ${step !== "three" ? "text-black" : ""}`}
        />
      </div>
